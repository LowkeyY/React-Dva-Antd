...
bin.user.RoleNavPanel = function(){
    var that=this;    

	this.clickEvent=function(clickNode){
		that.frames.get('state');
		var Role=that.frames.get('Role');
		var params={};
		var paramString=clickNode.prop.params.split('&');
		for(var i=0;i<paramString.length;i++){
			params[paramString[i].split('=')[0]]=paramString[i].split('=')[1];
		}       
		if(that.frames.get('state')!='list'){
			if(!Role.mainPanel.havePanel("RoleListPanel")){
				Role.roleListPanel = new bin.user.RoleListPanel(that.frames);
				Role.roleListPanel.frames=that.frames;
				Role.mainPanel.add(Role.roleListPanel.MainTabPanel);
			}
			Role.mainPanel.setActiveTab("RoleListPanel");
			Role.roleListPanel.showList(params);
		}else{
			Role.roleListPanel.showList(params);
		}   
	};
	bin.user.RoleNavPanel.superclass.constructor.call(this, {
            title: '部门导航'.loc(),
            region: 'west',
            split: true,
            width: 260,
...
Ext.extend(bin.user.RoleNavPanel, Ext.Panel, {
	init: function(){
		...
		conn.on('requestcomplete', function(conn, oResponse ){	
			...
			this.menuTree = new MenuTree(Tool.parseXML('<root _id="root"><forder _hasChild="1" event="event1"><e _id="'+orgJSON.id+'" _parent="root" title="'+name+'"  type="new"  params="dept_id='+orgJSON.id+'"  url="/bin/user/tree.jcp?dept_id=-1"   icon0="/themes/icon/all/chart_organisation.gif" icon1="/themes/icon/all/chart_organisation.gif"/></forder></root>'));
