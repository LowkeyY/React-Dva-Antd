...
utils.km.file.AuthFramePanel = function(params) {
	...
	this.AuthPanel = new utils.km.file.AuthPanel(this.params.instanceId);

	var ids = new Array();
	if (this.AuthPanel && this.AuthPanel.ds)
		this.AuthPanel.ds.each(function(rec) {
					ids.push(rec.get("user_id"));
				});

	var loader = new Ext.tree.TreeLoader({
				dataUrl : '/utils/km/file/roletree.jcp',
				requestMethod : "POST",
				baseParams : {
	...
	this.tree.on("render", function() {
				var ddrow = new Ext.dd.DropTarget(this.tree.getEl(), {
							...
							notifyDrop : function(dd, e, data) {
								this.grid2tree(data.grid, data.rowIndex,
										this.tree);
							}.createDelegate(this)
						});
			}, this);
...
utils.km.file.AuthFramePanel.prototype = {
