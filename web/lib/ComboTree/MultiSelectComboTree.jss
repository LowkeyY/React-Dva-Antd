...
/**
 * 支持多选的下拉列表树，用法如下： var tree=new lib.ComboTree.MultiSelectComboTree({
 * xtype:"multiselectcombotree", rootText:"测试站点",
 * url:'/lib/combotree/tree2.jcp', displayField : "text", valueField : "id",
 * rootVisible : true, parentField : "pid" });
 * 
 * 
 * {'root':[ {'text':'栏目1',pid:'-1','id':'lm1','cls':'folder'},
 * {'text':'栏目2','id':'lm2'',pid:'-1','cls':'folder'},
 * {'text':'稿件1','id':'gj1','leaf':true,'cls':'file',pid:'lm1'},
 * {'text':'稿件2','id':'gj2','leaf':true,'cls':'file',pid:'lm2'},
 * {'text':'稿件3','id':'gj3','leaf':true,'cls':'file',pid:'lm2'} ]}
 * 
 */
...
lib.ComboTree.MultiSelectComboTree = Ext.extend(Ext.form.ComboBox, {
	checkField : 'checked',
	displayField : "TEXT",
	valueField : "VALUE",
	...
	// lazyInit : false,
	/**
	 * @selectNodeModel all : 所有结点都可选中 exceptRoot ： 除根结点，其它结点都可选 folder :
	 *                  只有目录（非叶子和非根结点）可选 leaf ：只有叶子结点可选
	 */
	selectNodeModel : 'all',
	mode : "local",
	editable : false,
	...
	constructor : function() {
		...
		lib.ComboTree.MultiSelectComboTree.superclass.constructor.apply(this,
				arguments);
	...
	onCheckChange : function(node, checked) {
		...
		var r = this.findRecord(this.valueField,
				node.attributes[this.valueField]);
		if (r) {
	...
	formatData : function(data) {
		var len = data.length;
		var r = [], b = {}, p = this.parentField, v = this.valueField, d = this.displayField, s = this.rootValue
	...
	setValue : function(v) {
		if (typeof v != 'undefined') {
			...
			this.setRawValue(this.store.getCount() > 0 ? this
					.getCheckedDisplay() : this.value);
			if (this.hiddenField) {
...
Ext.reg('multiselectcombotree', lib.ComboTree.MultiSelectComboTree);