...
lib.ColorPicker.ColorPicker = Ext.extend(Ext.BoxComponent, {
	opacity : true,
	colorNames : {
		aliceblue : "F0F8FF",
		antiquewhite : "FAEBD7",
	...
	initComponent : function() {
		...
		lib.ColorPicker.ColorPicker.superclass.initComponent.apply(this, arguments);
		this.addEvents("select")
	},
	onRender : function() {
		lib.ColorPicker.ColorPicker.superclass.onRender.apply(this, arguments);
		this.body = this.body
				|| (this.container || (this.renderTo || Ext.DomHelper.append(
						Ext.getBody(), {}, true)));
		if (!this.el) {
			this.el = this.body;
			if (this.cls) {
	...
	renderComponent : function() {
		...
		this.hueSliderEl = Ext.DomHelper.append(this.body, {
					cls : "x-cp-slider",
					id : this.cpGetId("hueSlider")
				}, true);
		this.hueDD = new Ext.dd.DD(this.hueSliderEl, "huePicker");
		this.hueDD.constrainTo(this.hueEl.id, {
					top : -7,
					right : -3,
					bottom : -7,
					left : -3
		...
		Ext.get(this.hueSliderEl).moveTo(this.hueEl.getLeft() - 3,
				this.hueEl.getTop() - 7);
		this.rgbSliderEl = Ext.DomHelper.append(this.body, {
					cls : "x-cp-slider",
					id : this.cpGetId("rgbSlider")
				}, true);
		...
		this.rgbSliderEl.moveTo(this.rgbEl.getLeft() - 7, this.rgbEl.getTop()
						- 7);
		if (this.opacity) {
			this.opacitySliderEl = Ext.DomHelper.append(this.body, {
						cls : "x-cp-slider",
						id : this.cpGetId("opacitySlider")
					}, true);
			this.opacityDD = new Ext.dd.DD(this.opacitySliderEl.id,
					"opacityPicker");
			this.opacityDD.constrainTo(this.opacityEl.id, {
						top : -3,
						right : -7,
						bottom : -3,
						left : -7
			...
			this.opacityEl.on("mousedown", this.clickOpacityPicker
							.createDelegate(this));
			this.opacitySliderEl.moveTo(this.opacityEl.getRight() - 7,
					this.opacityEl.getTop() - 3);
			this.alpha = 255
		}
	...
	onDestroy : function() {
		lib.ColorPicker.ColorPicker.superclass.onDestroy.apply(this, arguments);
		Ext.destroyMembers(this, "formPanel", "inverseButton", "webSafeButton",
				"rgbEl", "hueEl", "opacityEl", "rgbSliderEl", "hueSliderEl",
				"opacitySliderEl", "previewEl", "opacityPreviewEl", "redCmp",
				"greenCmp", "blueCmp", "hueCmp", "satCmp", "valCmp", "hexaCmp")
	},
	...
	updateRGBPosition : function(x, y) {
		...
		this.rgbSliderEl.moveTo(this.rgbEl.getLeft() + x - 7, this.rgbEl
						.getTop()
						+ y - 7, (this.animateMove || true));
		this.updateColor()
	},
	updateHUEPosition : function(y) {
		...
		this.hueSliderEl.moveTo(this.hueSliderEl.getLeft(), this.hueEl.getTop()
						+ y - 7, (this.animateMove || true));
	...
	updateOpacityPosition : function(x) {
		...
		this.opacitySliderEl.moveTo(this.opacityEl.getLeft() + x - 7,
				this.opacitySliderEl.getTop(), (this.animateMove || true));
		this.updateColor()
	},
	clickRGBPicker : function(event, element) {
		this.updateRGBPosition(event.xy[0] - this.rgbEl.getLeft(), event.xy[1]
						- this.rgbEl.getTop())
	},
	...
	moveRGBPicker : function(event) {
		this.rgbDD.constrainTo(this.rgbEl.id, -7);
		this.updateRGBPosition(this.rgbSliderEl.getLeft()
						- this.rgbEl.getLeft() + 7, this.rgbSliderEl.getTop()
						- this.rgbEl.getTop() + 7)
	},
	moveHuePicker : function(event) {
		...
		this.updateHUEPosition(this.hueSliderEl.getTop() - this.hueEl.getTop()
				+ 7)
	},
	moveOpacityPicker : function(event) {
		...
		this.updateOpacityPosition(this.opacitySliderEl.getLeft()
				- this.opacityEl.getLeft() + 7)
	},
	updateRGBPicker : function(newValue) {
		this.updateMode = "click";
		this.rgbEl.setStyle({
					"background-color" : "#"
							+ this.rgbToHex(this.hsvToRgb(newValue, 1, 1))
				});
		this.updateColor()
	},
	updateColor : function() {
		...
		if (this.opacity) {
			this.opacityEl.setStyle({
						"background-color" : "#" + htmlC
					});
			Ext.getCmp(this.cpGetId("iOpacity")).setValue(Math.round(this.alpha
					/ 2.56));
		...
		this.inverseButton.setTooltip({
			title : "Inverse Color",
			text : '<div class="x-cp-tooltipcolorbox" style="background-color:#'
					+ invertC + ";color:#" + htmlC + '">#' + invertC + "</div>"
		});
		...
		this.hueSliderEl.moveTo(this.hueSliderEl.getLeft(), this.hueEl.getTop()
						+ this.getHPos(this.hueCmp.getValue()) - 7,
				(this.animateMove || true));
		this.rgbSliderEl.moveTo(this.rgbEl.getLeft()
						+ this.getSPos(this.satCmp.getValue() / 100) - 7,
				this.hueEl.getTop()
						+ this.getVPos(this.valCmp.getValue() / 100) - 7,
				(this.animateMove || true));
		this.rgbEl.setStyle({
					"background-color" : "#"
							+ this.rgbToHex(this.hsvToRgb(this.hueCmp
											.getValue(), 1, 1))
				})
	},
	setColor : function(c) {
		...
		this.alpha = s.alpha;
		if (this.opacity) {
			this.opacitySliderEl.moveTo(this.opacityEl.getLeft()
							+ (s.alpha / 255 * 181) - 7, this.opacitySliderEl
							.getTop(), (this.animateMove || true))
		}
	...
	splitAphaRgbHex : function(c) {
		...
		if (c.indexOf("rgb(") == 0) {
			...
			if (!/^[0-9a-fA-F]{6}$/.test(c)) {
				c = lib.ColorPicker.ColorPicker.colorNames[c];
				if (!Ext.isDefined(c)) {
					return
				}
			}
	...
	updateFromIRGB : function(input, newValue, oldValue) {
		this.updateMode = "rgb";
		var temp = this.rgbToHsv(this.redCmp.getValue(), this.greenCmp
						.getValue(), this.blueCmp.getValue());
		this.HSV = {
			h : temp[0],
			s : temp[1],
			v : temp[2]
	...
	updateFromIHSV : function(input, newValue, oldValue) {
		this.updateMode = "hsv";
		this.HSV = {
			h : this.hueCmp.getValue(),
			s : this.satCmp.getValue() / 100,
			v : this.valCmp.getValue() / 100
		};
		this.updateColor()
	},
	...
	websafe : function(r, g, b) {
		...
		return [this.checkSafeNumber(r), this.checkSafeNumber(g),
...
lib.ColorPicker.ColorDialog = Ext.extend(Ext.Window, {
			initComponent : function() {
				this.width = (!this.width || this.width < 353)
						? 353
						: this.width;
				this.applyDefaultsCP();
				lib.ColorPicker.ColorDialog.superclass.initComponent.apply(this,
						arguments)
			},
			onRender : function() {
				lib.ColorPicker.ColorDialog.superclass.onRender.apply(this, arguments);
				this.renderComponent()
			}
		});
Ext.applyIf(lib.ColorPicker.ColorDialog.prototype, lib.ColorPicker.ColorPicker.prototype);
lib.ColorPicker.ColorPanel = Ext.extend(Ext.Panel, {
			initComponent : function() {
				this.width = (!this.width || this.width < 333)
						? 333
						: this.width;
				this.applyDefaultsCP();
				lib.ColorPicker.ColorPanel.superclass.initComponent.apply(this,
						arguments)
			},
			onRender : function() {
				lib.ColorPicker.ColorPanel.superclass.onRender.apply(this, arguments);
				this.renderComponent()
			}
		});
Ext.applyIf(lib.ColorPicker.ColorPanel.prototype, lib.ColorPicker.ColorPicker.prototype);
Ext.reg("colorpicker", lib.ColorPicker.ColorPicker);
Ext.reg("colordialog", lib.ColorPicker.ColorDialog);
