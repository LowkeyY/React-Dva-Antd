...
dev.system.AuthPanel = function(frames,objectId){
	this.frames=frames;
	this.id=objectId;
	this.ds=new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: '/dev/system/auth.jcp?',
			method : 'GET'
		}),
	...
	dev.system.AuthPanel.superclass.constructor.call(this,{
		region:'east',
		collapsible: false,
		split:true,
		width:400,
...
Ext.extend(dev.system.AuthPanel, Ext.Panel, {
	...
	save: function(params){
			...
			Ext.Ajax.request({ 
				url:'/dev/system/auth.jcp',
				params:{ids:ids,objectId:this.id},
				method: 'post',  
				scope:this,
