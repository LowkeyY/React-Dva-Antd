...
dev.statistics.flashtongji = function(tabWidth) {

	var today = new Date();

	var ds = new Ext.data.JsonStore({
				method : 'GET',
				url : '/dev/statistics/flashtongji.jcp',
				root : 'authArray',
	...
	Ext.Ajax.request({
		...
		success : function(response, options) {
			...
			this.chartSample.write('sample6');
	...
	function creattu3(ds) {
		var today = new Date();
		var startdate6 = Ext.getCmp('startdate6').getRawValue();
		var enddate6 = Ext.getCmp('enddate6').getRawValue();
		ds.load({
					params : {
		...
		Ext.Ajax.request({
			...
			success : function(response, options) {
				...
				var xml = '<anychart> 	<settings> 		<animation enabled="True"/> 	</settings>   <charts>     <chart plot_type="CategorizedVertical">     	<chart_settings>     		<title>     			<text>Flash版本统计</text>     			<background enabled="false"/>     		</title>     		<axes>     			<x_axis tickmarks_placement="Center">     				<labels display_mode="Stager"/>     			</x_axis>     		</axes>     	</chart_settings>     	<data_plot_settings default_series_type="Spline">     		<line_series>     			<tooltip_settings enabled="true"> <format> Flash版本: {%Name} pv数量: {%YValue} </format>     			</tooltip_settings>     			<line_style>     				<line thickness="3"/> 	    		</line_style>     		</line_series>     	</data_plot_settings> 	    <data> 			<series name="Series 1"> 			 ';
