...
/**
 * javascript调试编辑工具,带有控制台.提供即时运行和错误查看
 * 
 */
lib.scripteditor.jssEditor = function(config) {
	this.config = config;
...
lib.scripteditor.jssEditor.prototype = {
	...
	getButtons : function() {
		var execute = function(btn) {
			btn.disable();
			javascriptContent = this.codeEditor.getValue();
			if (javascriptContent.replace("\\r", "") == "") {
				btn.enable();
				Ext.msg('警示'.loc(), '内容不得为空'.loc());
			}
			var params = this.config.params;
			params['content'] = javascriptContent;
			Ext.Ajax.request({
				url : '/lib/scripteditor/jssdebug.jcp',
				method : 'post',
