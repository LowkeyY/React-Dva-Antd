...
ExternalItems.haiwaizhishigongxiang.PluginUitls = function() {}

ExternalItems.haiwaizhishigongxiang.PluginUitls.prototype = {
	IconsMap : {
		'3gp' : '3gp.gif',
		'album' : 'album.gif',
	...
	addIcon2GridColumn : function(columnModel, addColumn, fromColumn) {
		...
		if (columnModel instanceof Ext.grid.ColumnModel && (index = columnModel.findColumnIndex(addColumn)) != -1) {
			var iconsMap = this.IconsMap, key = fromColumn, setIconFn = function(v, p, record) {
				var isSecrecy = Ext.isObject(record.get("IS_SECRECY")) ? record.get("IS_SECRECY").value : record.get("IS_SECRECY");
