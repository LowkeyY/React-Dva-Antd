...
dev.script.GridBase = function(config) {
    this.events = {
        "containerclick" : true,
    ...
    dev.script.GridBase.superclass.constructor.call(this, Ext.applyIf(config || {}, {
        
        plugins: [
            new dev.script.FilesGridDragSelector({dragSafe:true})
        ],        
          
        view: new dev.script.FilesGridView({
            forceFit:true,
            tpl: config.tpl,
...
Ext.extend(dev.script.GridBase, Ext.grid.GridPanel, {

   initEvents : function(){
      
	   dev.script.GridBase.superclass.initEvents.call(this);
