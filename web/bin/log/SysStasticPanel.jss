...
bin.log.SysStasticPanel = function() {
	...
	var titleClick = function(clickNode) {
		...
		if (clickNode.prop.objectType == '5') {
			...
			tabContainer.createChild({
						tag : 'img ',
						src : '/bin/log/image.jcp?id=' + objectId + '&firtim='
	...
	bin.log.SysStasticPanel.superclass.constructor.call(this, {
				region : 'center',
				collapsible : false,
				split : true,
				layout : 'fit',
...
Ext.extend(bin.log.SysStasticPanel, Ext.Panel, {
				 ...
				 if (activeTab.title == '模块日志统计图'.loc()) {
					...
					this.gride.getStore().load({
						...
						params : {
							...
							sTime : this.secondTime.getValue().add(Date.DAY, 1).format('Y/m/d'),
