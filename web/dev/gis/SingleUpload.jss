...
dev.gis.SingleUpload = function(config) {//alert();
    ...
    dev.gis.SingleUpload.superclass.constructor.call(this, config);
...
Ext.extend(dev.gis.SingleUpload, Ext.Window, {
    isUploading: false,
    startUpload: function(){  
		var saveParams={};
		saveParams["obj_type"] = this.obj_type;
        Ext.Ajax.request({
            url : this.uploadUrl,
            params : saveParams,
            method : 'POST',
    ...
    updateProgressResponse: function(stat) {    
        ...
        if (stat.CurrentStatus == dev.gis.StatusCode.Error) {
            this.cancelUpload();
