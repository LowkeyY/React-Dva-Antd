...
ExternalItems.haiwaizhishigongxiang.spacePage.pluginsDownloadWeek = function() {
	this.id = "ExternalItems.haiwaizhishigongxiang.spacePage.pluginsDownloadWeek";
	this.init();
}
ExternalItems.haiwaizhishigongxiang.spacePage.pluginsDownloadWeek.prototype = {
	...
	refresh : function(){
		...
		var chart = this.chart , mainPanel = this.mainPanel , data = this.data;
			...
			if(!mainPanel.el || !mainPanel.el.dom)
				return;
			Ext.Ajax.request({
					url : '/ExternalItems/haiwaizhishigongxiang/spacePage/getDownloadWeek.jcp',
					method : 'POST',
					scope : this,
