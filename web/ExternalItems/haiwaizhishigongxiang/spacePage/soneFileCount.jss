...
ExternalItems.haiwaizhishigongxiang.spacePage.soneFileCount = function() {
	this.id = "ExternalItems.haiwaizhishigongxiang.spacePage.soneFileCount";
	this.init();
}
ExternalItems.haiwaizhishigongxiang.spacePage.soneFileCount.prototype = {
	...
	refresh : function(){
		...
		var chart = this.chart , mainPanel = this.mainPanel , data = this.data;
			...
			if(!mainPanel.el || !mainPanel.el.dom)
				return;
			Ext.Ajax.request({
					url : '/ExternalItems/haiwaizhishigongxiang/spacePage/soneFileCount.jcp',
					method : 'POST',
					scope : this,
