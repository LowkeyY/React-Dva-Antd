...
dev.quality.AuthFramePanel= function(frames,params){
	...
	this.AuthPanel = new dev.quality.InstanceAuthPanel(this.frames,this.params.instanceId,this.params.parent_id);
	...
	var loader=new Ext.tree.TreeLoader({
			dataUrl:'/dev/quality/roletree.jcp',
			requestMethod:"POST",
...
dev.quality.AuthFramePanel.prototype={
	init:function(instanceId,qualityId){
		this.AuthPanel.instanceId=instanceId;
		this.AuthPanel.qualityId=qualityId;
		root=this.tree.root;
		root.expand();
		this.AuthPanel.ds.reload({
			params: {instanceId:instanceId} 
		});
		this.frames.get("qualityInstance").mainPanel.setStatusValue(["数据质量权限管理".loc()]);
	},
