Ext.ns('lib.GridSummary');

lib.GridSummary.GridSummary = function(config) {
        Ext.apply(this, config);
};

Ext.extend(lib.GridSummary.GridSummary, Ext.util.Observable, {
    init : function(grid) {
        this.grid = grid;
        ...
        for (var i = 0, len = cfg.length; i < len; i++) { // loop through all columns in ColumnModel
            ...
            if (cf.summaryType) {
                for (var j = 0, jlen = rs.length; j < jlen; j++) {
                    ...
                    data[cname] = lib.GridSummary.GridSummary.Calculations[cf.summaryType](r.get(cname), r, cname, data, j);
                }
            }
        }
...
Ext.reg('gridsummary', lib.GridSummary.GridSummary);
 ...
 */
lib.GridSummary.GridSummary.Calculations = {
...
lib.GridSummary.HybridSummary = Ext.extend(lib.GridSummary.GridSummary, {
    /**
     * @private
     * @param {Object} rs
     * @param {Object} cs
    ...
    calculate : function(rs, cs){
            gdata = this.getSummaryData();
        return gdata || lib.GridSummary.HybridSummary.superclass.calculate.call(this, rs, cs);
    },
...
Ext.grid.HybridSummary = lib.GridSummary.HybridSummary;