...
bin.workflow.SendListPanel = function(frames){
	this.frames=frames;
    this.xg = Ext.grid;
   ...
   this.ds = new Ext.data.Store({
    ...
    this.ds.setDefaultSort('申请日期'.loc(), 'desc');
				...
				var tempParams=this.ds.baseParams;
		 		parentPanel.returnFn=function(){
					...
					if(!Workflow.mainPanel.havePanel("SendListPanel")){
						var sendListPanel = new bin.workflow.SendListPanel(this.frames);
						Workflow.mainPanel.add(sendListPanel.MainTabPanel);
						sendListPanel.showList(tempParams);
					}
...
bin.workflow.SendListPanel.prototype={
