...
ExternalItems.haiwaizhishigongxiang.spacePage.ProjectFileCountBili = function() {
	this.id = "ExternalItems.haiwaizhishigongxiang.spacePage.ProjectFileCountBili";
	this.init();
}
ExternalItems.haiwaizhishigongxiang.spacePage.ProjectFileCountBili.prototype = {
	...
	refresh : function(){
		...
		var chart = this.chart , mainPanel = this.mainPanel , data = this.data;
			...
			if(!mainPanel.el || !mainPanel.el.dom)
				return;
			Ext.Ajax.request({
					url : '/ExternalItems/haiwaizhishigongxiang/spacePage/ProjectFileCountBili.jcp',
					method : 'POST',
					scope : this,
