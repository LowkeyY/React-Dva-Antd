...
function Completed ({ data }) {
  return (
    <div className={styles.sales}>
      <div className={styles.title}>师资力量</div>
      <ResponsiveContainer minHeight={360}>
        <AreaChart data={data}>
          ...
          />
          <Area type="monotone" dataKey="学生人数" stroke={color.grass} fill={color.grass} strokeWidth={2} dot={{ fill: '#fff' }} activeDot={{ r: 5, fill: '#fff', stroke: color.green }} />
          <Area type="monotone" dataKey="教师人数" stroke={color.sky} fill={color.sky} strokeWidth={2} dot={{ fill: '#fff' }} activeDot={{ r: 5, fill: '#fff', stroke: color.blue }} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  )
