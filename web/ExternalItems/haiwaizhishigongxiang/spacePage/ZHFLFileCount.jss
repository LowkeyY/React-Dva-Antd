...
ExternalItems.haiwaizhishigongxiang.spacePage.ZHFLFileCount = function() {
	this.id = "ExternalItems.haiwaizhishigongxiang.spacePage.ZHFLFileCount";
	this.init();
}
ExternalItems.haiwaizhishigongxiang.spacePage.ZHFLFileCount.prototype = {
	...
	refresh : function(){
		...
		var chart = this.chart , mainPanel = this.mainPanel , data = this.data;
			...
			if(!mainPanel.el || !mainPanel.el.dom)
				return;
			Ext.Ajax.request({
					url : '/ExternalItems/haiwaizhishigongxiang/spacePage/ZHFLFileCount.jcp',
					method : 'POST',
					scope : this,
