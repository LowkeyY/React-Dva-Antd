bin.menu.NavPanel = function(frames) {
	this.frames = frames;

	var strTitle = '菜单管理'.loc();
	if (frames.win) {
		strTitle = '根菜单'.loc();
	} else {
		this.title = '菜单导航'.loc();
		this.tbar = ["->", {
			...
			handler : function(btn) {
				...
				var tree = new bin.menu.NavPanel({
							win : true
						});
	...
	this.menuTree = new MenuTree(Tool
			.parseXML('<root _id="root"><forder _hasChild="1" event="event1"><e _id="0" _parent="root" title="'
					+ strTitle
					+ '"  program="portal"  params="isSystem=y&amp;id=0&amp;portal_id=0" url="/bin/menu/tree.jcp?id=0&amp;level=1&amp;root=&amp;view=create"/></forder></root>'));
	...
	this.clickEvent = function(clickNode) {
		...
		if (clickNode.prop.program == "menu") {
			if (!Menu.mainPanel.havePanel("menuBase")) {
			...
			if (clickNode.prop.params) {
				...
				conn.on('requestcomplete', function(conn, oResponse) {
							...
							if (!Menu.mainPanel.havePanel("topMenuBase")) {
								using("dev.system.TopMenuPanel");
								Menu.topMenu = new dev.system.TopMenuPanel(
										this.frames, Menu, 'menu', params,
										utJSON);
			...
			if (clickNode.prop.params) {
				...
				conn.on('requestcomplete', function(conn, oResponse) {
							...
							if (!Menu.mainPanel.havePanel("topMenuBase")) {
								using("dev.system.TopMenuPanel");
								Menu.topMenu = new dev.system.TopMenuPanel(
										this.frames, Menu, 'topMenu', params,
										utJSON);
								Menu.mainPanel.add(Menu.topMenu.MainTabPanel);
							}
							Menu.mainPanel.setActiveTab("topMenuBase");
	...
	bin.menu.NavPanel.superclass.constructor.call(this, {
				region : 'west',
				split : true,
				width : 225,
				collapsible : true,
...
Ext.extend(bin.menu.NavPanel, Ext.Panel, {
			init : function() {
