...
bin.km.file.AuthPanel = function(frames){

	this.frames=frames;
	this.archive_id=-1;
	this.ds=new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: '/bin/km/file/auth.jcp?',
			method : 'GET'
		}),
	...
	bin.km.file.AuthPanel.superclass.constructor.call(this,{
		region:'east',
		collapsible: false,
		split:true,
		width:400,
...
Ext.extend(bin.km.file.AuthPanel, Ext.Panel, {
	...
	save: function(params){
			...
			if(this.archive_id!=300){
					Ext.Ajax.request({ 
						url:'/bin/km/file/auth.jcp',
						params:{ids:ids,archive_id:this.archive_id},
						method: 'post',  
						scope:this,
				...
				Ext.msg("error",'不能针对非档案室节点赋权!'.loc());
			}
	},
	load:function(archive_id){
