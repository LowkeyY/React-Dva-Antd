...
ExternalItems.haiwaizhishigongxiang.spacePage.ProjectFileSizeBili = function() {
	this.id = "ExternalItems.haiwaizhishigongxiang.spacePage.ProjectFileSizeBili";
	this.init();
}
ExternalItems.haiwaizhishigongxiang.spacePage.ProjectFileSizeBili.prototype = {
	...
	refresh : function(){
		...
		var chart = this.chart , mainPanel = this.mainPanel , data = this.data;
			...
			if(!mainPanel.el || !mainPanel.el.dom)
				return;
			Ext.Ajax.request({
					url : '/ExternalItems/haiwaizhishigongxiang/spacePage/ProjectFileSizeBili.jcp',
					method : 'POST',
					scope : this,
