...
etc.install.BasePanel = function(frames, from) {
	...
	this.formDS = new Ext.data.Store({
				...
				reader : new Ext.data.JsonReader({}, ["system_id",
								"system_name", "system_pname", "system_type",
								"current_stat", "system_desc", "creator",
	...
	var el = new Ext.ux.IFrameComponent({
				id : 'installConsole',
				url : '/bin/blank.html',
...
etc.install.BasePanel.prototype = {
	onButtonClick : function(item) {
		...
		if (item.id == 'install') {
			...
			if (frm.isValid()) {
				...
				form.action = '/etc/install/install.jcp';
				form.submit();
			} else {
