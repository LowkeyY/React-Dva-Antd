 ...
 */
Ext.ns('lib.RowEditorGrid');

/**
 * @class lib.RowEditorGrid.RowEditor
...
Ext.ux.grid.RowEditor = Ext.extend(Ext.Panel, {
	...
	saveText : '保存'.loc(),
	cancelText : '取消'.loc(),
	commitChangesText : '您需要保存或取消您修改过的数据'.loc(),
	errorText : '错误'.loc(),
	...
	onRowDblClick : function(g, rowIndex, e) {
		//this.startEditing(rowIndex, false); //不好做修改权限的判断。
		//this.doFocus.defer(this.focusDelay, this, [e.getPoint()]);
	},
	...
	positionButtons : function() {
		if (this.btns) {
			var g = this.grid, h = this.el.dom.clientHeight, view = g.getView(), scroll = view.scroller.dom.scrollLeft, bw = this.btns
					.getWidth(), width = Math.min(g.getWidth(), g
							.getColumnModel().getTotalWidth());
	...
	showTooltip : function(msg) {
		...
		var v = this.grid.getView(), top = parseInt(this.el.dom.style.top, 10), scroll = v.scroller.dom.scrollTop, h = this.el
				.getHeight();
...
// 兼容老代码
Ext.ns("lib.RowEditorGrid");
lib.RowEditorGrid.RowEditor = Ext.ux.grid.RowEditor;