...
bin.workflow.FinishListPanel = function(frames){
	this.frames=frames;
    this.xg = Ext.grid;
				...
				var tempParams=this.ds.baseParams;
		 		parentPanel.returnFn=function(){
					...
					if(!Workflow.mainPanel.havePanel("FinishListPanel")){
						var finishListPanel = new bin.workflow.FinishListPanel(this.frames);
						Workflow.mainPanel.add(finishListPanel.MainTabPanel);
						finishListPanel.showList(tempParams);
					}
...
bin.workflow.FinishListPanel.prototype={
