...
ExternalItems.userauth.UserAuthNavPanel = function(User){
	this.orgId="";
	that=this;
	this.clickEvent=function(clickNode){
		...
		if(this.frames.get('state')!='list'){
			if(!User.mainPanel.havePanel("UserAuthListPanel")){
				User.UserAuthListPanel =new ExternalItems.userauth.UserAuthListPanel(User);
				User.UserAuthListPanel.frames=this.frames;
				User.mainPanel.add(User.UserAuthListPanel.MainTabPanel);
			}else{
				User.mainPanel.getStatusBar().hide();
			}
			User.mainPanel.setActiveTab("UserAuthListPanel");
			User.UserAuthListPanel.showList(params);
		}else{
			User.UserAuthListPanel.showList(params);
		}
	};
	...
	ExternalItems.userauth.UserAuthNavPanel.superclass.constructor.call(this, {
            title: '部门导航'.loc(),
            region: 'west',
            split: true,
            width: 260,
...
Ext.extend(ExternalItems.userauth.UserAuthNavPanel, Ext.Panel, {
	init: function(){
