...
bin.workflow.WorkflowAuthPanel = function(frames,instanceId,workflowId){
	this.frames=frames;
	this.instanceId=instanceId;
	this.workflowId=workflowId;
	this.ds=new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: '/bin/workflow/auth.jcp?',
			method : 'GET'
		}),
	...
	bin.workflow.WorkflowAuthPanel.superclass.constructor.call(this,{
		region:'east',
		collapsible: false,
		split:true,
		width:400,
...
Ext.extend(bin.workflow.WorkflowAuthPanel, Ext.Panel, {
	...
	save: function(params){
			...
			Ext.Ajax.request({ 
				url:'/bin/workflow/auth.jcp',
				params:{ids:ids,instanceId:this.instanceId,workflowId:this.workflowId},
				method: 'post',  
				scope:this,
