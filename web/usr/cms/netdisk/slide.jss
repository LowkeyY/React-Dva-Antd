...
skin : {
	controls:
		'<div class="highslide-controls"><ul>'+
			'<li class="highslide-previous" style="display:none;">'+
				'<a href="#" title="{Ext.hstz.lang.previousTitle}">'+
				'<span>{Ext.hstz.lang.previousText}</span></a>'+
			'</li>'+
			'<li class="highslide-play" style="display:none;">'+
			...
			'<li class="highslide-next">'+
				'<a href="#" title="{Ext.hstz.lang.dowTitle}" id="downBtn" target="_Blank" style="">'+
				'<span>{Ext.hstz.lang.dowTitle}</span></a>'+
			'</li>'+
...
disable: function(btn) {
	...
	if (!/disabled$/.test(a.className) && a.id!="downBtn") a.className += ' disabled';
},
hitSpace: function() {
...
Ext.hstz.Thumbstrip = function(slideshow) {
	...
	for (var i = 0; i < group.length; i++) {
		...
		(function(){
			...
			downBtn.href="/usr/cms/netdisk/downFile.jcp?fileId="+a.id+"&path="+a.path+"&file_name="+a.name+"&file_type="+a.file_type;
