...
bin.dataservice.ReportAuthPanel = function(frames,folder_id){

	this.frames=frames;
	this.folder_id=folder_id;
	this.ds=new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: '/bin/dataservice/auth.jcp?',
			method : 'GET'
		}),
	...
	bin.dataservice.ReportAuthPanel.superclass.constructor.call(this,{
		region:'east',
		collapsible: false,
		split:true,
		width:400,
...
Ext.extend(bin.dataservice.ReportAuthPanel, Ext.Panel, {
	...
	save: function(params){
			...
			if(this.folder_id!=300){
				...
				Ext.msg("error",'不能针对跟报表根目录赋权!'.loc());
			}
	}
});
