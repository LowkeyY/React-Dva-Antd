...
//=========================
	...
	this.getClassArray = function(list){
		...
		for(var i=0;i<list.length;i++){
			...
			if(clazz == null)
				throw new Error('集合里缺少原型类:'.loc()+list[i]);
			else
				arr.push(clazz);
		}
		return arr;
	...
	this.defineCSS = function(xmlDoc){
		...
		for(var i=0;i<cssList.length;i++){
			cssNode=cssList[i];
			var fullName = cssNode.getAttribute("name");
...
_p.toString = function(){
	if(window.debug){
		return ['用户信息'.loc(),this.user,'系统信息'.loc(),this.sys,'调试信息'.loc(),this.debug].join("\n\n");
	}else
		return this.user;
}
////////////////////////////////////////////////////////////// init
if(!window.DIALOG){
	window.setManager = function(){
		eval("Jsvm="+window.core.__CLASS.toString());
		window.core = new Jsvm();
		window.core.copy(window.core);
	}
	window.onbeforeunload = function(){
		if(window == window.core.coreWindow){
			var win = window.core.getWindow();
			if(win == null)
				window.opener = window.core;
			else{
				window.opener = win;
				window.core.removeChild(win);
				win.setManager();
			}
		}else
			window.core.removeChild(window);
	}
}
System = function(){
	var loaded = false;
	...
	var importClass = function(list){
		var noLoaded = window.core.getNoneClassName(list);
		var paramsNoLoad;
		if(noLoaded.length >0){
			paramsNoLoad=noLoaded.join(',');
		}
		if(noLoaded.length >0){
			window.core.defineClass(Tool.loadLib(paramsNoLoad));	
		}
		var classCodes = window.core.getClassArray(noLoaded);
		var proto = new Array();
	...
	var importCSS = function(list){
		var noLoaded = window.core.getNoneCSSName(list);
		if(noLoaded.length >0){
			for(var i=0;i<noLoaded.length;i++){
				window.core.defineCSS(Tool.loadCSS(noLoaded[i]));	
			}
		}
		var cssCodes = window.core.getCSSArray(noLoaded);
		var proto = new Array();
	...
	window.getId = function(){return window.core.getId();}
	window.getOpener = function(){return window.core.getOpener();}
}
...
Tool.getXML = function(url){
	...
	if(xmlHTTP.status != 200){
		var err = new KingleException('数据提交错误,程序被迫中止'.loc(),'数据载入错误'.loc()+' url='+url);
...
Tool.postXML = function(url,xmlDoc){
	...
	if(xmlHTTP.status != 200){
		...
		err.user += '\n '+'服务信息:'.loc()+xmlDoc.selectSingleNode("user").text;
		err.debug = new Array();
		if(xmlDoc.selectSingleNode("sys"))
			err.debug.push(xmlDoc.selectSingleNode("sys").text);
		if(xmlDoc.selectSingleNode("debug"))
...
Tool.loadLib = function(list){
	...
	try{
		xmlDoc.async = false;	
		xmlDoc.load("/jsvm/JavaScript.jcp?"+list);
	}catch(e){alert(e.message);return false}
	if(!xmlDoc.documentElement){
		throw new Error('类库载入错误'.loc()+list);
	}
	return xmlDoc.documentElement;*/
	return Tool.load("/jsvm/JavaScript.jcp?"+list);
}
Tool.loadCSS = function(list){
	/*var xmlDoc = Tool.XMLDOM;
	xmlDoc.async = false;	
	xmlDoc.load("/jsvm/CSS.jcp?"+list);
	if(!xmlDoc.documentElement){
		throw new Error('类库载入错误'.loc()+list);
	}
	return xmlDoc.documentElement;
	*/
	return Tool.load("/jsvm/CSS.jcp?"+list);
}
Tool.load = function(url){
	var xmlDoc;
	try{
		xmlDoc= Tool.XMLDOM;
		xmlDoc.async = false;	
		xmlDoc.load(url);
	}catch(e){//mustbe Google chrome
...
Tool.disableEvent = function(win){
	if(!win)
		win = window;
	...
	for(var i=0;i<colNameList.length;i++){
		...
		if(colName.charAt(0) =="/"){
			...
			for(var j=0;j<this.height;j++){
				try{
					temp[j] = eval(colName);
				}catch(e){alert(colName+"\n"+colNameList[i]+'算法输入错误!'.loc());continue inner;}
			}
...
Mover.mousedown = function(){
	if(window.event.button == 1){
		...
		this.__eventX = window.event.screenX;
		this.__eventY = window.event.screenY;
		window.event.cancelBubble = true;
	}
}
Mover.mousemove = function(){
	try{
		this._moverNode.draging(this,(window.event.screenX - this.__eventX),(window.event.screenY - this.__eventY));
	}catch(e){}
}

Mover.mouseup = function(){
	try{
		this._moverNode.dragEnd(this,(window.event.screenX - this.__eventX),(window.event.screenY - this.__eventY));
	}catch(e){}
	Mover.release(this);
}
...
window.popup = function(url,param,focus,w,h,resizable){
	...
	if(w) width = "dialogWidth:"+w+"px;";
	if(h) height="dialogHeight:"+h+"px;";
	if(resizable)
		resize = "resizable:1;";
	if(focus)
		return window.showModalDialog(url,[window.core,param],"help:no;status:no;"+width+height+resize);
	else
		return window.showModelessDialog(url,[window.core,param],"help:no;status:no;"+width+height+resize);
}
UserStore=function(store){
	this.flag="tz_";
	if(Ext.isIE){
		if(!store){
			alert('无法载入'.loc()+' tree_store');
			return false;
		}
		this.store=store;
...
_p.getAttribute=function(name){
		return (Ext.isIE)?this.store.getAttribute(name):get_cookie(this.flag+name);
}
