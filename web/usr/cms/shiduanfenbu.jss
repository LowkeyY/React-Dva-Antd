...
usr.cms.shiduanfenbu=function(){
	

	Ext.apply(usr.cms.shiduanfenbu.prototype,{
		...
		var ds=new Ext.data.JsonStore({
					method: 'GET',
					url : '/usr/cms/shiduanfenbu.jcp',
					root : 'authArray',
					fields : ["shijian", "pv", "uv","ip","shijianjian"],
					remoteSort : true
		});
...
var grid = new Ext.grid.GridPanel({
        ...
        enableDragDrop:true,
        columns:[
        	{header:"时间",width:100,dataIndex:"shijian",sortable: false},
		...
		Ext.Ajax.request({
									...
									success : function(response, options) {
										...
										for(var i=0;i<list.length;i++){
											xml+="<point name=\""+list[i].shijianjian+"\" y=\""+list[i].pv+"\"/>"
										}
