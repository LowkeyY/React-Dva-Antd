...
dev.statistics.fenbianlvtongji = function(tabWidth) {

	var today = new Date();

	var ds = new Ext.data.JsonStore({
				method : 'GET',
				url : '/dev/statistics/fenbianlvtongji.jcp',
				root : 'authArray',
	...
	Ext.Ajax.request({
		...
		success : function(response, options) {
			...
			this.chartSample.write('sample5');
	...
	function creattu3(ds) {
		var today = new Date();
		var startdate5 = Ext.getCmp('startdate5').getRawValue();
		var enddate5 = Ext.getCmp('enddate5').getRawValue();
		ds.load({
					params : {
		...
		Ext.Ajax.request({
			...
			success : function(response, options) {
				...
				var xml = '<anychart> 	<settings> 		<animation enabled="True"/> 	</settings>   <charts>     <chart plot_type="CategorizedVertical">     	<chart_settings>     		<title>     			<text>分辨率统计</text>     			<background enabled="false"/>     		</title>     		<axes>     			<x_axis tickmarks_placement="Center">     				<labels display_mode="Stager"/>     			</x_axis>     		</axes>     	</chart_settings>     	<data_plot_settings default_series_type="Spline">     		<line_series>     			<tooltip_settings enabled="true"> <format> 分辨率: {%Name} pv数量: {%YValue} </format>     			</tooltip_settings>     			<line_style>     				<line thickness="3"/> 	    		</line_style>     		</line_series>     	</data_plot_settings> 	    <data> 			<series name="Series 1"> 			 ';
