...
ExternalItems.haiwaizhishigongxiang.spacePage.oneLeveFileCount = function() {
	this.id = "ExternalItems.haiwaizhishigongxiang.spacePage.oneLeveFileCount";
	this.init();
}
ExternalItems.haiwaizhishigongxiang.spacePage.oneLeveFileCount.prototype = {
	...
	refresh : function(){
		...
		var chart = this.chart , mainPanel = this.mainPanel , data = this.data;
			...
			if(!mainPanel.el || !mainPanel.el.dom)
				return;
			Ext.Ajax.request({
					url : '/ExternalItems/haiwaizhishigongxiang/spacePage/oneLeveFileCount.jcp',
					method : 'POST',
					scope : this,
