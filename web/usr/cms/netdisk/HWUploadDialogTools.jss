...
loadcss("usr.cms.netdisk.swfupload.upload");
loadcss("usr.cms.netdisk.swfupload.icons");
HWUploadDialogTools = Ext.extend(Ext.Window, {
	...
	listeners : {
		'close' : function(win) {
			...
			if (p = Ext.getCmp(this.formId)) {
				...
				if (fp && (fi = fp.frameIndex) && fi.west
						&& (t = Ext.getCmp(fi.west))) {
					if ((t = t.getComponent(0)) && t.nav) {

						var nav;
						if (t && (nav = t.nav))
							nav.getTree().loadSelfNodeWithLevel(p.param.dataId,
									nav.clickEvent.createDelegate(nav));
	...
	constructor : function(config) {
		...
		// var width=900;
		// var height=800;
		HWUploadDialogTools.superclass.constructor.call(this, Ext.applyIf(
						config || {}, {
							title : '文件上传',
							closeAction : 'close',
							layout : 'border',
	...
	startUpload : function() {
		if (this.swfupload) {
			/*
			 * this.swfupload.uploadStopped = false; var post_params =
			 * this.swfupload.settings.post_params; post_params.path =
			 * encodeURI(this.scope.currentPath + '\\'); if
			 * (!this.propertyForm.getForm().isValid()) { Ext.msg("warn",
			 * "请填写红色必输项."); return; } var p = Ext.apply({}, post_params,
			 * Ext.apply({}, this.paramModels,
			 * this.propertyForm.form.getValues())); //var p = Ext.apply({},
			 * this.paramModels, this.propertyForm.form.getValues()); for(var
			 * att in p){ p[att] = encodeURI(p[att]) };
			 * this.swfupload.setPostParams(p); this.swfupload.startUpload();
			 * //this.swfupload.uploadStart({});
			 */
			this.swfupload.uploadStopped = false;
			var post_params = Ext
					.apply({}, this.swfupload.settings.post_params);
			...
			if (files.length > 0)
				Ext.each(this.getQueueFiles(), function(rec) {
					swf.addFileParam(rec.fileId, "userCustomInfoName",
							encodeURI(rec.infoName));
						// infos[rec.name] = rec.infoName;
					});
			var p = Ext.apply({}, Ext.apply({}, this.paramModels,
							this.propertyForm.form.getValues()), post_params);
			for (var att in p) {
	...
	getQueueFiles : function() {
		...
		this.fileList.getStore().each(function(rec) {
					if (rec.get("fileState") === SWFUpload.FILE_STATUS.QUEUED)
						files.push({
									name : rec.get("fileName").replace(
											/\s|\./g, ""),
									infoName : rec.get("fileInfoName"),
									id : rec.id,
	...
	optListAndInput : function(name) {
		...
		if ((field = this.propertyForm.getForm().findField("FILE_INFO_NAME"))
				&& (index = this.fileList.colModel
						.findColumnIndex("fileInfoName")) != -1) {
	...
	formatFileSize : function(_v, celmeta, record) {
		return '<div id="fileSize_' + record.data.fileId
				+ '" style="color:gray;">' + Ext.util.Format.fileSize(_v)
				+ '</div>';
	},
	formatFileName : function(_v, cellmeta, record) {
		return '<div id="fileName_' + record.data.fileId
				+ '" style="color:gray;">' + _v + '</div>';
	},
	formatFileInfoName : function(_v, cellmeta, record) {
		return '<div id="fileInfoName_' + record.data.fileId + '">' + _v
				+ '</div>';
	},
		...
		if (frm && (field = frm.findField("FILE_CLASS"))) {
			...
			Ext.Ajax.request({
						...
						success : function(response, options) {
							...
							Ext.msg(result.success ? "info" : "warn",
									result.message);
						}
					});
		}
	...
	onFileQueueError : function(file, errorCode, message) {
		...
		try {
			...
			if (errorCode == SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE) {
				var msg = message.indexOf("cannot be accessed") != -1
						? "或文件决绝访问"
						: "";
	...
	onUploadSuccess : function(file, serverData, eve) {
		...
		if (isSuccess) {
			var record = eve.fileList.getStore().getById(Ext.getDom('fileId_'
